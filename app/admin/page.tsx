  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <QuickAction href="/admin/clientes" icon="👤" label="Clientes" desc="ABM clientes" />
        <QuickAction href="/admin/silos" icon="🌾" label="Silos" desc="Stock cereal" />
        <QuickAction href="/admin/hacienda" icon="🐄" label="Hacienda" desc="Rodeo" />
        <QuickAction href="/admin/configuracion" icon="⚙️" label="Configurar" desc="Datos del campo" />
      </div>
      ...
      <div className="bg-white border border-[var(--border)] rounded-2xl p-8 shadow-sm">
        <div className="text-center space-y-3 py-8">
          ...
          <p className="text-[var(--fg-muted)] max-w-md mx-auto">
            ...
            la sección{' '}
            <Link href="/admin/configuracion" className="text-[var(--primary)] font-semibold hover:underline">
              Configuración
            </Link>.
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white border border-[var(--border)] rounded-2xl p-5 shadow-sm">
          ...
          {ctx.rol === 'admin_productor' && (
            <div className="mt-4 pt-4 border-t border-[var(--border)]">
              <Link href="/admin/configuracion" className="text-sm text-[var(--primary)] hover:underline font-medium">
        ...
        <div className="bg-white border border-[var(--border)] rounded-2xl p-5 shadow-sm">
          ...
          <dl className="text-sm space-y-1.5">
            <DLRow label="Plan"><span className="capitalize">{productor.plan}</span></DLRow>
            <DLRow label="Estado"><EstadoChip estado={productor.estado_suscripcion} /></DLRow>
...
function AlertaSuscripcion({ alerta }: { alerta: { tipo: 'trial' | 'pago' | 'vencido'; dias: number; fecha: string } }) {
  const colores = {
  ...
  if (alerta.tipo === 'trial') {
    icon = '🎁';
    mensaje = alerta.dias === 0 ? 'Hoy es el último día de tu prueba.' : `Te quedan ${alerta.dias} días de prueba gratuita.`;
  } else if (alerta.tipo === 'pago') {
    icon = '💳';
    mensaje = alerta.dias === 0 ? 'Hoy vence tu próximo pago.' : `En ${alerta.dias} días vence tu próximo pago (${formatFecha(alerta.fecha)}).`;
  } else {
    icon = '⚠️';
...
function QuickAction({ href, icon, label, desc }: { href: string; icon: string; label: string; desc: string }) {
  return (
    <Link href={href} className="bg-white border border-[var(--border)] rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-[var(--primary)] transition group">
...
function EstadoChip({ estado }: { estado: string }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-semibold capitalize ${map[estado] ?? map.cancelada}`}>
      {estado}
    </span>
  );
}
