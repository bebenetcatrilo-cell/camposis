...
function buildHTML(pres: Presupuesto, items: Item[], productor: Productor): string {
  ...
  const ivaRow =
    Number(pres.iva_porcentaje) > 0
      ? `
    <div class="total-row">
      <span>IVA (${Number(pres.iva_porcentaje)}%)</span>
      <span>$${fmt(Number(pres.iva_monto))}</span>
    </div>`
      : '';
  ...
  const notasHTML = pres.notas
    ...
    <div class="section">
      ...
      <div class="notas">${pres.notas.replace(/\n/g, '<br/>')}</div>
    </div>`
    : '';
  ...
  const conceptoHTML = pres.concepto
    ...
    <div class="section">
      ...
      <div style="font-size:13px;color:#374151">${pres.concepto}</div>
    </div>`
    : '';
...
<div class="page">
  ...
  <div class="totales">
    <div class="totales-inner">
      <div class="total-row">
        <span>Subtotal</span>
        <span>$${fmt(Number(pres.subtotal))}</span>
      </div>
      ${ivaRow}
      <div class="total-final">
        <span>TOTAL</span>
        <span>$${fmt(Number(pres.total))}</span>
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    Documento sin valor fiscal · Presupuesto N° ${numFmt} · ${fmtFecha(pres.fecha)}
  </div>
...
export function ImprimirBtn({ pres, items, productor }: Props) {
  function imprimir() {
    const html = buildHTML(pres, items, productor);
